...
%DIF ADD main.tex                  Thu Apr 29 17:14:07 2021
...
\begin{definition}
   ...
   \end{equation}
   that returns a matrix $\mat{A}$ such that $\mat{A}$ is a valid covariance ($\mat{A}\succ 0$ and $\mat{A}=\mat{A}^\top$) and for each of its eigenvalues $e\in\mathsf{eig}(\mat{A})$, there exists a negligible function \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite[Def. 3.4]{katzIntroductionModernCryptography2008} }\hskip0pt%DIFAUXCMD
}\DIFaddend $\eta$ such that $e\leq\eta(\kappa)$.
\end{definition}

With the terminology above, we can \DIFdelbegin \DIFdel{now }\DIFdelend introduce the security notion that captures the formal requirements of the estimation problem we want to solve.
...
\begin{equation}
   ...
   \end{bmatrix}^\top\,,
\end{equation}
for any matrix $\mat{A}$ such that $\mat{A}\mat{A}^\top=\mat{Z}$. \DIFaddbegin \DIFadd{In addition, for the correct removal of noise terms $\vec{p}_k$ at the privileged estimator, index information $k$ is needed when sensor data is lost or delayed. While estimation over the internet may use the indexing information already present in TCP/IP, for the remainder of the work we consider the case when all measurements arrive, in order, and neglect additional index information for the sake of simplicity.
}\DIFaddend 
...
Recalling the security notion in section \ref{subsec:crypto_problem}, we \DIFdelbegin \DIFdel{now show how }\DIFdelend \DIFaddbegin \DIFadd{aim to show how it is met by }\DIFaddend our single privilege level estimation scheme\DIFdelbegin \DIFdel{in section \ref{subsec:adding_privilege_noise}, given conditions on $\mathcal{M}_S$ and $\mathcal{M}_M$, meets the notion for a computable series $\mat{D}_k$, $k>0$, dependent on the noise parameter $\mat{Z}$}\DIFdelend \DIFaddbegin \DIFadd{. Prior to the proof sketch, we look at our scheme in the context of a formal privileged estimation scheme with model contraints and give some relevant optimality properties}\DIFaddend .
...
\end{equation}
for any estimator $\mathcal{A}$ following definition \ref{def:estimator} and any Gaussian, linear, and time-invariant models $\mathcal{M}_S$ and $\mathcal{M}_M$. This leads us into the \DIFdelbegin \DIFdel{sketch proof }\DIFdelend \DIFaddbegin \DIFadd{security proof sketch}\DIFaddend .
