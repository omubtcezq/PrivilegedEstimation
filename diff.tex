...
%DIF ADD main.tex                  Fri Apr 30 16:55:30 2021
...
, where intermittent sending is proposed }\DIFdelend \DIFaddbegin \DIFadd{Alternatively, in \mbox{%DIFAUXCMD
\cite{murguiaInformationTheoreticPrivacyChaos2020,leongUseArtificialNoise2018a}}\hskip0pt%DIFAUXCMD
, additive noises from a synchronized chaotic system and at the physical layer, respectively, are used }\DIFaddend to increase eavesdropper estimation error\DIFdelbegin \DIFdel{while using a secure feedback channel to ensure better estimation for trusted parties. }\DIFdelend \DIFaddbegin \DIFadd{. These works, however, do not give formal cryptographic guarantees on the security which they provide. }\DIFaddend Our contribution in this work considers this context of privileged and unprivileged estimation and is comprised of a \DIFaddbegin \DIFadd{novel }\DIFaddend formal definition of privileged state estimation, crucial for cryptographic security, before proposing a scheme that provides privileged levels of estimation without reliance on additional secure channels\DIFdelbegin \DIFdel{or estimator feedback}\DIFdelend . We accompany the method with a cryptographic proof sketch and simulation results.
...
To formalize this\DIFdelbegin \DIFdel{definition}\DIFdelend , we introduce the following notations and definitions. We assume the existence of an arbitrary process (not necessarily Gaussian or linear) following a known system model exactly, with the state at time step $k$ denoted \DIFdelbegin \DIFdel{by }\DIFdelend $\vec{x}_k\in\mathbb{R}^n$ and model parameters $\mathcal{M}_S$. Similarly, we assume the existence of a means of process measurement following a known measurement model exactly, with the measurement at time step $k$ denoted \DIFdelbegin \DIFdel{by }\DIFdelend $\vec{y}_k\in\mathbb{R}^m$ and model parameters $\mathcal{M}_M$. We \DIFdelbegin \DIFdel{can }\DIFdelend now define a \textit{privileged estimation scheme} as a pair of \DIFdelbegin \DIFdel{algorithms }\DIFdelend \DIFaddbegin \DIFadd{probabilistic algorithms, }\DIFaddend $(\mathsf{Setup},\mathsf{Noise})$\DIFaddbegin \DIFadd{, }\DIFaddend given by
...
To allow meeting the cryptographic notion in section \mbox{\ref{subsec:crypto_problem}}, we focus on Gaussian, linear and time-invariant models where the minimum achievable error covariance is easily computable, and \DIFdelbegin \DIFdel{produce }\DIFdelend \DIFaddbegin \DIFadd{add }\DIFaddend a keystream of pseudorandom Gaussian noise. The keystream and added noise are given next.
...
\end{equation}
where \DIFdelbegin \DIFdel{$\vec{v}_k\sim \mathcal{N}(\vec{0},\mat{R}+\mat{E}_i)$}\DIFdelend \DIFaddbegin \DIFadd{$\vec{v}^{(i)}_k\sim \mathcal{N}(\vec{0},\mat{R}+\mat{E}_i)$}\DIFaddend , with
...
\DIFdelend \DIFaddbegin \begin{split}
      ...
      &\quad\bigl(\mat{H}(\mat{F}\mat{P}_{k-1}^{(l)}\mat{F}^\top + \mat{Q})\mat{H}^\top + \mat{R}\bigr)^{-1}\mat{H}\Bigr)\cdot\\
...
\end{equation}
for any estimator $\mathcal{A}$ following definition \ref{def:estimator} and any Gaussian, linear, and time-invariant models $\mathcal{M}_S$ and $\mathcal{M}_M$. This leads us \DIFdelbegin \DIFdel{into the sketch proof }\DIFdelend \DIFaddbegin \DIFadd{to the security proof sketch}\DIFaddend .

\DIFaddbegin \begin{theorem}\label{thm:prooved_thm}
   \DIFadd{Our single privilege estimation scheme in section \ref{subsec:adding_privilege_noise} meets $\{\mat{D}_1,\mat{D}_2,\dots\}$-Covariance Privilege for Models $\mathcal{M}_S$ and $\mathcal{M}_M$, for a computable series $\mat{D}_k$, $k>0$ dependent on a noise parameter $\mat{Z}$, when $\mathcal{M}_S$ and $\mathcal{M}_M$ are Gaussian, linear and time-invariant.
}\end{theorem}

\DIFaddend \subsubsection{Proof Sketch}
...
\DIFdelend \DIFaddbegin \begin{split}
      ...
      &\quad\bigl(\mat{H}(\mat{F}\mat{P}_{k-1}^{\prime(l)}\mat{F}^\top + \mat{Q})\mat{H}^\top + \mat{R}+\mat{Z}\bigr)^{-1}\mat{H}\Bigr)\cdot\\
...
We \DIFdelbegin \DIFdel{have considered }\DIFdelend \DIFaddbegin \DIFadd{consider }\DIFaddend two simulations, both following the same two-dimensional constant-velocity system model, given by
\begin{equation*}
   \mat{F}\! = \!\!
   \begin{bmatrix}
      1 & 0.5 & 0 & 0\\
      ...
      0 & 0 & 0 & 1
   \end{bmatrix}
\DIFaddbegin \DIFadd{\!\!}\DIFaddend \text{ and }\DIFaddbegin \DIFadd{\!
   }\DIFaddend \mat{Q}\! = \!\frac{1}{10^3}\!\cdot\!
   \begin{bmatrix}
      0.4 & 1.3 & 0 & 0\\
      1.3 & 5.0 & 0 & 0\\
      0 & 0 & 0.4 & 1.3\\
   ...
   \end{bmatrix}\!,
\end{equation*}
with differing measurement models. In all cases, estimators were initialized with the same initial conditions, equal to the true starting condition of the system they were estimating\DIFaddbegin \DIFadd{, }\DIFaddend with initial error covariance $\mat{0}$.
