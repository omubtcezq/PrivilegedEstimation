...
That's correct. As with any encryption method relying on a stream cipher, index information is cruital for the correct decryption of sent data, and in our case, the correct removal of added noise. While some networked systems relying on an additional transfer protocol such as TCP/IP provide index (and thus time step k) information in sent packets, we have considered the simplified case where all sent measurements arrive, in order, and have neglected any additional index information. We failed to mention this in the work and thank you for pointing it out; a pargraph has now been added in Section III.B which clarifies the need for index information to be sent alongside measurements and states the context which we consider.
