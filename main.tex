...
\end{definition}% does covariance above specifically consider the sameples y, or is it taken as k->inf (may be more than a negligible difference with concrete measurements?)!! - NO (prob. taken over Noise function, means its correct)[leaving this here to confuse me less later]
...
% our scheme in terms of the setup and noise function above should ge here somwhere no?
% init estimate is in the pub variable, but D_i should be defined so that it's tru for any init guess
...
As we use a cryptographically pseudorandom keystream, noisey measurements $\vec{y}^\prime_k$ are indistinguiashable from measurements following the modified measurement model () exactly.
...
As we know from the CRLB proof, any estimators with the two models () and () will at least differ by $\mat{D}_k$ at time $k$. Given an estimator


In addition to the security definitions and proof above, we stress two aditional implicit assumptions when dealing with real physical processes 
...
assumption that uniform floating points are uniform enough
