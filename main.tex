...
\author{Marko Ristic$^{1}$, Benjamin Noack$^{1}$, \IEEEmembership{Member, IEEE}, and Uwe D. Hanebeck$^{2}$, \IEEEmembership{Fellow, IEEE}% <-this % stops a space
\thanks{$^{1}$Marko Ristic and Benjamin Noack are with the Autonomous Multisensor Systems Group (AMS), Institute for Intelligent Cooperating Systems (ICS), Otto von Guericke University Magdeburg (OVGU), Germany {\tt\small \{marko.ristic, benjamin.noack\}@ovgu.de}}%
\thanks{$^{2}$Uwe D. Hanebeck is with the Intelligent Sensor-Actuator-Systems Laboratory (ISAS), Institute for Anthropomatics, Karlsruhe Institute of Technology (KIT), Germany {\tt\small uwe.hanebeck@kit.edu}}%
}
