...
The security notion desired would require that for any subset of corrupted estimators, and therefore the knowledge of any subset of different privilege level secret keys $s \subset \{\mathsf{sk}_i,\,0 \leq i < N\}$ and noisy meausmrents, an estimator given true measurements exists, such that the difference between the corrupted subset's error covariance and its own is at least $\mat{D}^{(s)}_k$ at time $k$.
...
As well as showing the theoretical security of our scheme, we have simulated the estimation problem using estimators which achieve the CRLB error covariances for varying privileges.

We have considered simulations for both bounded and unbounded error covariances to demonstrate the method. The process model followed a two-dimensional constant velocity model, with
\begin{equation*}
   \mat{F}=
   \begin{bmatrix}
      1 & 0.5 & 0 & 0\\
...
Simulations were implemented in the Python programming language, using the AES block cipher in CTR mode as a crytographically secure stream cipher (AES-CTR) [].

In figure (), the average error covariance over 1000 runs has been plotted for each privilege level (solid) and the special cases where estimator held all the secret key (dashed green) and none of them (dashed red).
